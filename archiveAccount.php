            ...
            else
            {
              ...
              $sqlAccount = "SELECT accountNo, firstname, lastname, middlename, houseNo, streetName, city, region, ZIP , mobile, a.areaName , a.areaID FROM accounts ac
                ...
                else
                {
                    ...
                    $sql_archive = "UPDATE Accounts set statusID ='2' WHERE accountNo = ".$_REQUEST['accountNo'];
                    $conn->query($sql_archive) or die (mysqli_error($conn));

                    $sql_archive_map ="UPDATE map set status = 'Archived' Where accountNo= ".$_REQUEST['accountNo'];
                    $conn->query($sql_archive_map) or die (mysqli_error($conn));
...
<script src="https://maps.googleapis.com/maps/api/js?key=AIzaSyCxLcBhYr5nz356Li4jgaT1nwXB8CkXgz8&callback=myMap"></script>
     ...
     <div class="col-lg-5">
           ...
           <button name="submit" type="submit" class="btn btn-success btn-lg pull-right" onclick="return confirm('Are you sure you want to Delete?');">
